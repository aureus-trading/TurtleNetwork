...
class TransferTxFromProtoSuite extends BaseTransactionSuite {
  ...
  private val scriptText =
    s"""
      ...
      |    let assetId = if (!transferTx.assetId.isDefined()) then {"TN"} else {transferTx.assetId.value().toBase58String()}
      |    let feeAssetId = if (!transferTx.feeAssetId.isDefined()) then {"TN"} else {transferTx.feeAssetId.value().toBase58String()}
      |[
  ...
  test("TransferTransaction with Waves from proto bytes") {
    ...
    sender.getDataByKey(dAppAddress, "assetId").value shouldBe "TN"
    sender.getDataByKey(dAppAddress, "feeAssetId").value shouldBe "TN"
