...
class InvokeSelfPaymentSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("prerequisite: set contract") {
    ...
    val sourceV4 =
      """{-# STDLIB_VERSION 4 #-}
        ...
        |  let id = if asset == "TN" then unit else fromBase58String(asset)
  ...
  test("V4: can't send tokens to itself from a script") {
    for (args <- List(
           List(CONST_STRING("TN").explicitGet()),
  ...
  test("V3: still can pay itself") {
    sender.invokeScript(caller, dAppV3Address, Some("paySelf"), List(CONST_STRING("TN").explicitGet()), waitForTx = true)
