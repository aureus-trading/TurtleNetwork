...
class TransactionFeeSpec extends FreeSpec with WithDomain {
  "invoke script" - {
    ...
    "with issued assets" in withDomain(settingsWithFeatures(BF.BlockV5, BF.Ride4DApps, BF.SynchronousCalls)) { d =>
      ...
      val invokeScript = InvokeScriptTransaction
        .selfSigned(2.toByte, dAppSigner, dAppAddress, None, Seq.empty, 0.06.waves, Asset.Waves, ntpTime.getTimestamp())
        .explicitGet()
    ...
    "with verifier" - {
      "with complexity <= 200" in withDomain(settingsWithFeatures(BF.BlockV5, BF.Ride4DApps, BF.SynchronousCalls)) { d =>
        ...
        val invoke = InvokeScriptTransaction
          .selfSigned(TxVersion.V2, sender, dappAccount.toAddress, None, Seq.empty, 0.1.waves, Asset.Waves, ntpTime.getTimestamp())
          .explicitGet()
      ...
      "with complexity > 200" in withDomain(settingsWithFeatures(BF.BlockV5, BF.Ride4DApps, BF.SynchronousCalls)) { d =>
        ...
        val invoke = InvokeScriptTransaction
          .selfSigned(2.toByte, sender, dappAccount.toAddress, None, Seq.empty, 0.1.waves, Asset.Waves, ntpTime.getTimestamp())
          .explicitGet()
  ...
  "transfer transaction" - {
    "with sponsored asset fee" in withDomain(DomainPresets.ScriptsAndSponsorship.withActivationPeriod(1)) { d =>
      ...
      d.appendBlock(TxHelpers.genesis(TxHelpers.defaultAddress, 1100.waves))
      d.appendBlock(issue)
      d.appendBlock(
        SponsorFeeTransaction.selfSigned(TxVersion.V1, TxHelpers.defaultSigner, issue.asset, Some(1L), 1000.waves, ntpTime.getTimestamp()).explicitGet()
      )
    ...
    "smart asset with smart account" in withDomain(DomainPresets.RideV4) { d =>
      ...
      d.appendBlock(TxHelpers.genesis(TxHelpers.defaultAddress, 2100.waves))
      d.appendBlock(issue)
