...
class SpentComplexitySpec
  ...
  "Invocation" - {
    ...
    "counts asset script complexity when smart asset payment is attached" in withDomain(settings) { d =>
      ...
      val invokeTx = InvokeScriptTransaction
        .selfSigned(2.toByte, recipient, sender.toAddress, None, Seq(InvokeScriptTransaction.Payment(50_00L, issue.asset)), 1000_0000L, Asset.Waves, ntpTime.getTimestamp())
        .explicitGet()
      ...
      d.appendBlock(
        ...
        issue,
        transferAsset,
        SetScriptTransaction.selfSigned(2.toByte, sender, Some(contract), 1000_0000L, ntpTime.getTimestamp()).explicitGet(),
        invokeTx
      )
